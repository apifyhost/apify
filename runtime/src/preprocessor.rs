...
pub fn preprocessor(flow: &str, base_path: &Path, print_flow: bool) -> Result<String, Vec<String>> {
    ...
    let flow = preprocessor_eval(&flow);
...
fn preprocessor_directives(flow: &str, base_path: &Path) -> (String, Vec<String>) {
...
fn preprocessor_eval(flow: &str) -> String {
...
fn preprocessor_modules(flow: &str) -> Result<String, Vec<String>> {
    ...
    let escaped_flow = escape_yaml_exclamation_values(flow);

    // Parse o YAML para extrair módulos disponíveis
    let parsed: Value = match serde_yaml::from_str(&escaped_flow) {
        Ok(val) => val,
        Err(_) => return Ok(flow.to_string()), // Se não conseguir parsear, retorna o original
    };
    ...
    if available_modules.is_empty() {
        return Ok(flow.to_string()); // Sem módulos para transformar
    }
    ...
    let mut parsed_mut: Value = match serde_yaml::from_str(&escaped_flow) {
        Ok(val) => val,
        Err(_) => return Ok(flow.to_string()),
    };
...
fn unescape_yaml_exclamation_values(yaml: &str) -> String {
    let regex = match Regex::new(r"__APIFY_ESCAPE__(!\w[^\s]*)") {
        Ok(re) => re,
        Err(_) => return yaml.to_string(),
    };
