...
impl CRUDHandler {
    ...
    async fn handle_create(
                    ...
                    crate::schema_generator::RelationType::HasOne => {
                        ...
                        if let Some(nested_data) = data_map.remove(&relation.field_name) {
                            if let Value::Object(_) = nested_data {
                                ...
                                nested_single_relations.push((relation.field_name.clone(), nested_data));
                            }
                        }
                    }
        ...
        if !nested_relations.is_empty() || !nested_single_relations.is_empty() {
            if let Some(parent_id) = inserted_id {
                    ...
                    if let Some(relation) = schema.relations.iter().find(|r| r.field_name == field_name) {
                        ...
                        for mut item in items {
                            if let Value::Object(ref mut item_map) = item {
                                ...
                                if let Some(identity) = ctx.extensions.get::<ConsumerIdentity>() {
                                    if let Some(target_schema) = self.api_generator.get_table_schema(&relation.target_table) {
                                        for col in &target_schema.columns {
                                            if col.auto_field && (col.name == "createdBy" || col.name == "updatedBy") {
                                                item_map.insert(col.name.clone(), Value::String(identity.name.clone()));
                                            }
                                        }
                                    }
                                }
                                ...
                                let item_result = self.db_manager.insert(&relation.target_table, item_hashmap).await?;
                ...
                tracing::warn!("No parent ID found in insert result, cannot process nested relations");
            }
        }
    ...
    async fn fetch_with_relations(
        ...
        let record = results.into_iter().next().ok_or_else(|| {
            CRUDError::NotFoundError("Record not found".to_string())
        })?;
        ...
        for relation in &table_schema.relations {
            match relation.relation_type {
                ...
                crate::schema_generator::RelationType::HasOne => {
                    ...
                    let related = self
                        .db_manager
                        .select(&relation.target_table, None, Some(where_clause), Some(1), None)
                        .await
                        .unwrap_or_else(|_| Vec::new());
            ...
            if let Some(schema) = table_schema {
                ...
                for (field_name, new_items) in nested_relations {
                    if let Some(relation) = schema.relations.iter().find(|r| r.field_name == field_name) {
                        ...
                        let deleted = self.db_manager.delete(&relation.target_table, child_where).await.unwrap_or(0);
                        ...
                        for mut item in new_items {
                            if let Value::Object(ref mut item_map) = item {
                                ...
                                self.db_manager.insert(&relation.target_table, item_hashmap).await?;
                            }
                        }
                    }
                }
                ...
                for (field_name, new_item) in nested_single_relations {
                    if let Some(relation) = schema.relations.iter().find(|r| r.field_name == field_name) {
                        ...
                        let mut child_where = HashMap::new();
                        child_where.insert(relation.foreign_key.clone(), record_id.clone());
                        self.db_manager.delete(&relation.target_table, child_where).await.ok();
                ...
                for relation in &schema.relations {
                    match relation.relation_type {
                        ...
                        crate::schema_generator::RelationType::HasOne => {
                            ...
                            let deleted_count = self.db_manager
