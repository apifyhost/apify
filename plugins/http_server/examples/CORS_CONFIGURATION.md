...
See `examples/api-cors/api-cors.APIFY` for a comprehensive example with CORS enabled for cross-origin requests.

### 2. API without CORS

See `examples/api-no-cors/api-no-cors.APIFY` for an API that runs without any CORS headers (default behavior).
