...
pub async fn http_request(
    ...
    let default_user_agent = {
        let string_bool = std::env::var("APIFY_HTTP_REQUEST_USER_AGENT_DISABLE")
            .unwrap_or_else(|_| "false".to_string());

        match string_bool.as_str() {
            "true" => None,
            _ => {
                let default_user_agent = std::env::var("APIFY_HTTP_REQUEST_USER_AGENT")
                    .unwrap_or_else(|_| "APIFY HTTP Request".to_string())
                    .to_string();
