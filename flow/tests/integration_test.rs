...
mod integration_tests {
    ...
    fn test_error_types() {
        ...
        assert!(format!("{transform_error}").contains("Transform error"));

        let pipeline_error = FlowError::PipelineNotFound(42);
        assert!(format!("{pipeline_error}").contains("Pipeline not found"));

        let step_error = FlowError::StepNotFound(1, 2);
        assert!(format!("{step_error}").contains("Step not found"));
    }
}
