...
async fn wait_for_ready_tcp(host: &str, port: u16, timeout: Duration) -> Result<(), Box<dyn std::error::Error>> {
...
"#);
  ...
  let resp_no_key = client.get(&ready_url).send().await?;
    ...
    eprintln!("Unauthorized probe status={} body={}", status_no_key, body_no_key);
    assert_eq!(status_no_key, 401, "users endpoint should require key_auth (got {} body {})", status_no_key, body_no_key);

    // With key -> 200 (empty list or whatever response)
  let resp_key = client.get(&ready_url)
    ...
    assert_eq!(status_key, 200, "users endpoint should succeed with valid key (got {} body {})", status_key, body_key);
