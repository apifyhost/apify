...
async fn users_crud_flow() -> Result<(), Box<dyn std::error::Error>> {
    let temp = TempDir::new()?; let dir = temp.path();
...
"#);
    let cfg_path = dir.join("config.yaml"); fs::write(&cfg_path, cfg)?;
    ...
    let mut child = TokioCommand::new(bin)
        ...
        .env("APIFY_THREADS", "1")
        .arg("-c").arg(cfg_path.to_string_lossy().to_string())
        .spawn()?;
    ...
    // Create
    let r = client.post(format!("{}/users", base)).header(key.0, key.1)
        .json(&serde_json::json!({"name":"Alice"}))
        .send().await?;
    assert_eq!(r.status(), 200);

    // List and pick the inserted id
    let r = client.get(format!("{}/users", base)).header(key.0, key.1).send().await?;
    assert_eq!(r.status(), 200);
    let arr: Value = r.json().await?;
    let arr = arr.as_array().cloned().unwrap_or_default();
    let user = arr.iter().find(|v| v.get("name").and_then(|n| n.as_str()) == Some("Alice"))
        .cloned().expect("inserted user not found");
    let id = user.get("id").and_then(|v| v.as_i64()).unwrap_or_else(|| {
        user.get("id").and_then(|v| v.as_str()).and_then(|s| s.parse::<i64>().ok()).expect("id not parsable")
    });

    // Get by id
    let r = client.get(format!("{}/users/{}", base, id)).header(key.0, key.1).send().await?;
    assert_eq!(r.status(), 200);

    // Update
    let r = client.put(format!("{}/users/{}", base, id)).header(key.0, key.1)
    ...
    // Delete
    let r = client.delete(format!("{}/users/{}", base, id)).header(key.0, key.1).send().await?;
    assert_eq!(r.status(), 200);
