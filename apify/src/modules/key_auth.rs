...
use super::{Module, ModuleOutcome, ConsumerIdentity, error_response};
...
impl Module for KeyAuthModule {
    ...
    fn run(&self, phase: Phase, ctx: &mut RequestContext, state: &Arc<AppState>) -> ModuleOutcome {
        ...
        if header_val.is_none() { return ModuleOutcome::Respond(error_response(StatusCode::UNAUTHORIZED, "missing api key")); }
        let key = header_val.unwrap();
        if let Some(consumer) = state.lookup_consumer_by_key(key) {
            ctx.extensions.insert(ConsumerIdentity { name: consumer.name.clone() });
