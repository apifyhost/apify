...
pub async fn handle_request(
    ...
    if let Some(crud_handler) = &state.crud_handler {
        ...
        let body = if matches!(method.as_str(), "POST" | "PUT" | "PATCH") {
            let body_bytes = http_body_util::BodyExt::collect(body_stream).await?.to_bytes();
            if body_bytes.is_empty() {
                None
            } else {
                match serde_json::from_slice::<Value>(&body_bytes) {
                    Ok(value) => Some(value),
                    Err(_) => return Ok(create_error_response(StatusCode::BAD_REQUEST, "Invalid JSON body")),
                }
            }
        } else {
            None
        ...
        match crud_handler.handle_request(
            method.as_str(),
            path,
            path_params,
            query_params,
            ...
            Err(CRUDError::NotFoundError(_)) => {
                Ok(create_error_response(StatusCode::NOT_FOUND, "Resource not found"))
            }
            ...
            Err(CRUDError::DatabaseError(_)) => {
                Ok(create_error_response(StatusCode::INTERNAL_SERVER_ERROR, "Database error"))
            }
        }
    } else {
...
fn extract_path_params(_state: &AppState, _path: &str, _method: &hyper::Method) -> HashMap<String, String> {
